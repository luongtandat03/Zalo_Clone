...
import { getChatHistory, getPinnedMessages, searchMessages, connectWebSocket, sendMessage, uploadFile, recallMessage, deleteMessage, forwardMessage } from '../api/messageApi';
...
const ChatRoomScreen = ({ route, navigation }) => {
        ...
        try {
          ...
          const client = await connectWebSocket(
            ...
            (message) => {
              ...
              if (message.type === 'TEXT' || message.type === 'IMAGE' || message.type === 'VIDEO' || message.type === 'FILE' || message.type === 'FORWARD') {
                setMessages((prev) => {
  ...
  const loadChatHistory = async (retryCount = 0) => {
    ...
    try {
      ...
      const updatedChatHistory = chatHistory.map((msg) => {
        ...
        const isDeleted = deletedMessageIds.includes(msgId) || deletedIds.includes(msgId); // kết hợp state và async
      ...
      setMessages(deduplicateMessages(updatedChatHistory));
    } catch (error) {
  ...
  const loadPinnedMessages = async (retryCount = 0) => {
    ...
    try {
      ...
      console.log('Fetching pinned messages with params:', params); // Debug log
      const pinned = await getPinnedMessages(otherUserId || webSocketUserId, groupId, token);
      setPinnedMessages(messages.filter(msg => pinned.some(p => p.id === msg.id)));
      console.log('Pinned messages loaded:', pinned); // Debug log
      if (pinned && isMounted.current) {
        setPinnedMessages(pinned);
      } else if (retryCount < 5) {
        console.log(`Retry loadPinnedMessages, attempt ${retryCount + 1}`);
        setTimeout(() => loadPinnedMessages(retryCount + 1), 1000);
      }
    } catch (error) {
      console.error('Lỗi tải tin nhắn ghim:', error.response?.data || error.message);
      if (retryCount < 5) {
        console.log(`Retry loadPinnedMessages after error, attempt ${retryCount + 1}`);
        setTimeout(() => loadPinnedMessages(retryCount + 1), 1000);
      } else {
        console.warn('Không thể tải tin nhắn ghim sau nhiều lần thử, bỏ qua.');
        setPinnedMessages([]); // Đặt giá trị mặc định
      }
    }
  };
  ...
  const handleSearchMessages = async () => {
    ...
    try {
      const results = await searchMessages(otherUserId || webSocketUserId, groupId, searchInput, token);
      if (results.length > 0) {
        const firstMatch = results[0];
        const msgIndex = messages.findIndex((msg) => msg.id === firstMatch.id);
        if (msgIndex !== -1 && scrollViewRef.current) {
          scrollViewRef.current.scrollTo({ y: msgIndex * 60, animated: true });
          setHighlightedMessageId(firstMatch.id);
          if (!firstMatch.read && firstMatch.senderId !== webSocketUserId) {
            handleReadMessage(firstMatch.id);
          }
        }
      } else {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const destination = isGroupChat ? '/app/chat.send.group' : '/app/chat.send';
      const result = await sendMessage(destination, newMessage, token);
      if (!result) Alert.alert('Lỗi', 'Không thể gửi tin nhắn: WebSocket không hoạt động');
      setMessageInput('');
    } catch (error) {
  ...
  const handlePinMessage = async () => {
    ...
    try {
      ...
      console.log('Sending pin request:', request); // Debug log
  ...
  const handleUnpinMessage = async () => {
    ...
    try {
      ...
      console.log('Sending unpin request:', request); // Debug log
      const success = await sendMessage(destination, request, token);
      if (success) {
          ...
          {message.recalled ? (
            ...
            <>
              ...
              {message.type === 'IMAGE' && message.content && (
                <Image
                  source={{
                    uri: message.content.startsWith('http')
                      ? message.content
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.header}>
          ...
          {isGroupChat && (
            <TouchableOpacity style={styles.iconButton} onPress={() => { setMembers([]); fetchMembers(); }}>
          ...
          <View style={styles.friendListModalContainer}>
            <View style={styles.friendListModalContent}>
              ...
              {isLoadingFriends || (isGroupChat && isLoadingMembers) ? (
                ...
                <FlatList
                  data={[...friends, ...(isGroupChat ? members : [])]}
...
const styles = StyleSheet.create({
  ...
  messageList: { flex: 1, padding: 10, paddingTop: 20 },
  ...
  friendListModalContainer: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'flex-end' },
  ...
  friendListTitle: { fontSize: 18, fontWeight: 'bold' },
  friendList: { flexGrow: 0 },
  friendItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#eee' },
